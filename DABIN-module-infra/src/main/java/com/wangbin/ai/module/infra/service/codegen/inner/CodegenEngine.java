...
import com.wangbin.ai.framework.common.util.date.DateUtils;
...
import com.wangbin.ai.framework.common.util.string.StrUtils;
import com.wangbin.ai.framework.excel.core.annotations.DictFormat;
import com.wangbin.ai.framework.excel.core.convert.DictConvert;
import com.wangbin.ai.framework.excel.core.util.ExcelUtils;
import com.wangbin.ai.framework.mybatis.core.dataobject.BaseDO;
...
import static com.wangbin.ai.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.wangbin.ai.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 DABIN-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 DABIN-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.wangbin.ai.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/DABIN-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/DABIN-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "DABIN-module-${table.moduleName}/" + // 顶级模块
                "DABIN-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "DABIN-module-${table.moduleName}/" + // 顶级模块
                "DABIN-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "DABIN-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "DABIN-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "DABIN-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "DABIN-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
